...
import { Trash2, EyeOff, AlertTriangle, Eye } from 'lucide-react';
    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            {activeTab === 'posts' ? (
                ...
                <CommentList comments={comments} posts={posts} onHide={hideComment} onReply={replyComment} />
            )}
        </div>
    );
};
...
const PostList = ({ posts, onDelete }: { posts: Post[], onDelete: (id: string) => void }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {posts.map(post => (
            <div key={post.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col group hover:shadow-md transition-shadow">
                <Link to={`/admin/content/post/${post.id}`} className="block relative h-48 overflow-hidden">
                    <img src={post.images[0]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ...
                <div className="p-4 flex-1 flex flex-col">
                    ...
                    <Link to={`/admin/content/post/${post.id}`} className="block">
                        <h3 className="font-bold text-gray-800 mb-2 line-clamp-1 group-hover:text-primary transition-colors">{post.title}</h3>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                        ...
                        <div className="flex items-center gap-2">
                            <Link
                                to={`/admin/content/post/${post.id}`}
                                className="text-primary hover:bg-primary/5 p-2 rounded-lg flex items-center gap-1 text-xs font-medium transition-colors"
                            >
                                <Eye size={14} /> 查看详情
                            </Link>
                            <button
...
const CommentList = ({ comments, posts, onHide, onReply }: { comments: Comment[], posts: Post[], onHide: (id: string) => void, onReply: (id: string, text: string) => void }) => {
    return (
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between mb-2">
                                <div>
                                    ...
                                    <span className="text-xs text-gray-400">在帖子 {post?.title || '未知帖子'} ({comment.postId})</span>
                                </div>
